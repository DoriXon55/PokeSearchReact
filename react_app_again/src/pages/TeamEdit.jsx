...
const TeamEdit = ({ darkMode }) => {
  ...
  const addPokemonToTeam = async (pokemon) => {
    ...
    try {
      setIsUpdating(true);
      await teamsApi.addPokemonToTeam(id, pokemon.id, selectedPosition);
      ...
      setError('Nie udało się dodać Pokemona do drużyny.');
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Link to={`/teams/${id}`} className={`${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`}>
        &larr; Wróć do szczegółów drużyny
      </Link>
      
      <h1 className={`text-3xl font-bold mt-4 mb-6 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
        Edytuj drużynę
      </h1>
      
      <div className={`rounded-lg shadow-lg overflow-hidden mb-8 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        <div className="p-6">
          ...
          <div className="mb-4">
            ...
            <div className="flex">
              ...
              <button
                ...
                disabled={!teamName.trim() || teamName === team.name || isUpdating}
                className={`ml-2 px-4 py-2 rounded-lg ${
                  darkMode
                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                    : 'bg-blue-500 hover:bg-blue-600 text-white'
                } ${(!teamName.trim() || teamName === team.name || isUpdating) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Aktualizuj nazwę
              </button>
            </div>
      ...
      <div className={`rounded-lg shadow-lg overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        <div className="p-6">
          <h2 className={`text-xl font-semibold mb-6 ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>
            Pokemony w drużynie
          </h2>
          ...
          {selectedPosition !== null ? (
            <div className="mb-6">
              ...
              {searchError && (
                <p className={`text-center my-4 ${darkMode ? 'text-red-400' : 'text-red-600'}`}>
                  {searchError}
                </p>
              )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-4">
                  {searchResults.map(pokemon => (
                    ...
                    >
                      ...
                      />
                      <p className={`text-center capitalize mt-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                        {pokemon.name}
                      </p>
                ...
                >
                  ...
                  {pokemon ? (
                    <>
                      ...
                      <h3 className={`text-center capitalize mt-1 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
